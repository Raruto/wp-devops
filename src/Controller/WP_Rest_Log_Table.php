 ...
 */
class WP_Rest_Log_Table extends \WP_List_Table
{
	/**
	 * DB_VERSION - holds current database version,
	 * checked on plugin update when changes are made to
	 * the structure of tables
	 */
	const DB_VERSION = "0.0";

	/**
	 * DB_TABLE - holds current log table name
	 */
	const DB_TABLE = "ghu_logs";
	...
	{
		global $wpdb;
		$table_name = $wpdb->prefix . self::DB_TABLE; // do not forget about tables prefix
	...
	function prepare_items()
	{
		...
		public static function update_db_table()
		{
			...
			if ($installed_ver != self::DB_VERSION) {
				...
				if($wpdb->get_var("show tables like '$table_name'") != $table_name) {
					...
					update_site_option('ghu_db_version', self::DB_VERSION);
				}
			}
		}
		...
		{
			global $wpdb;
			$sql = "DROP TABLE {$wpdb->prefix}" . self::DB_TABLE;
			$wpdb->query($sql);
		}
